"""v0.5.58 — current public-truth and containment contract."""
...
def test_version_and_tool_availability_are_exact_across_discovery_surfaces():
    ...
    assert http_server.SERVER_VERSION == "0.5.60"
    assert health["version"] == "0.5.60"
...
def test_registry_manifest_has_same_current_availability_truth():
    ...
    assert manifest["version"] == "3.37.0"
    assert "v0.5.60" in manifest["description"]
